// GTA V ванильные транспортные средства с категориями
// Данные из vehicles_hashes_with_category.txt

import vehicleCategoriesData from './vehicle-categories.json'

export interface GTAVVehicle {
  name: string
  displayName: string
  hash: number
  category: string
...
// Хэши транспортных средств GTA V
const vehicleHashes: Record<string, number> = {
  adder: 3078201489,
  airbus: 1283517198,
  airtug: 1560980623,
  akula: 1181327175,
...
// Создаем маппинг имя -> категория из JSON данных
const nameToCategory: Record<string, string> = {}
for (const category of vehicleCategoriesData.categories) {
  for (const vehicleName of category.vehicles) {
    nameToCategory[vehicleName] = category.name
  }
}

/**
 * Получить список GTA V транспортных средств с категориями
 */
export function getGTAVVehicles(): GTAVVehicle[] {
  return Object.entries(vehicleHashes).map(([name, hash]) => ({
    name,
    displayName: name.charAt(0).toUpperCase() + name.slice(1),
    hash,
    category: nameToCategory[name] || 'Unknown',
    isGTAV: true
  }))
}

/**
 * Найти GTA V транспорт по имени
 */
export function findGTAVVehicle(name: string): GTAVVehicle | undefined {
  const vehicles = getGTAVVehicles()
  return vehicles.find(v => v.name.toLowerCase() === name.toLowerCase())
}

/**
 * Найти GTA V транспорт по хэшу
 */
export function findGTAVVehicleByHash(hash: number): GTAVVehicle | undefined {
  const vehicles = getGTAVVehicles()
  return vehicles.find(v => v.hash === hash)
}

/**
 * Получить список категорий
 */
export function getGTAVCategories(): string[] {
  return [...new Set(Object.values(nameToCategory))].sort()
}

/**
 * Получить машины по категории
 */
export function getGTAVVehiclesByCategory(category: string): GTAVVehicle[] {
  return getGTAVVehicles().filter(v => v.category === category)
}
